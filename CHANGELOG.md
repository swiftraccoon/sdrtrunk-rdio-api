...
No tagged release has been published yet. Until the first release, the package
version in `pyproject.toml` is the source of truth and all changes remain under
Unreleased.
...
- **Patch (X.Y.Z)**: Backwards-compatible fixes and security patches

### Release Process

1. Update version in `pyproject.toml`
2. Regenerate `uv.lock` and update CHANGELOG.md with the release date
3. Merge the release commit to `main`
4. Create and push a signed or annotated `vX.Y.Z` tag
5. Let the release workflow verify the tag and create the GitHub release
...
[Unreleased]: https://github.com/swiftraccoon/sdrtrunk-rdio-api/commits/main
